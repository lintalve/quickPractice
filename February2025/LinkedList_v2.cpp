/*
 ...
 The most impoortant part is to know and deeply and thorrorly the mechanism
 of link list
 */
...
#include <iostream>
#include <string>
#include <fstream>    //includes both std::ifstream and std::ofstream
...
int main(int argc, const char* argv[]) {
    ...
    if(file) {
        ...
        while(std::getline(file, string)) {
            strings.push(new std::string(string));
        }
        std::cout << strings.length() << std::endl;
        const std::string* strPtr;
        while((strPtr = strings.pop())) std::print("{}\n", *strPtr);
        //while((strPtr = strings.pop())) std::cout << *strPtr << std::endl;
    }
    file.close();
    return 0;
}
