...
//{L} fibonacci #include "fibonacci.h"
#include "../require.h"
#include <iostream>
#include <string>
using namespace std;
...
public:
    ...
    public:
        ...
        iterator& operator+=(int amount) {
            require(index + amount < s.top, "IntStack::iterator::operator+=() "
...
int main() {
    ...
    for(int i = 0; i < 20; i++) is.push(fibonacci(i));   //nice, concentrat, level 2-3
    
    cout << "Traverse the whole IntStack\n";
    
    IntStack::iterator it = is.begin();
    while(it != is.end()) cout << it++ << endl;     //nice code, all internals produce logic, but on the next level of abstraction
