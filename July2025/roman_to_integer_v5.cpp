...
int roman_to_integer(const std::string& roman) {
    ...
    int j {};
    std::print("theInt is {}\n", theInt);
    while(j < roman.length()-1) {      //checks if starting index of the pair is 1
        //another formulation from the discrete math logic
        
        
        std::print("index is {}\n", roman.length() -1 -j);
        //at first we put two items in the pair, then we check whether the this pair is the last one
        ...
        //cheks if the pair is the last pair
        std::print("R is {}\n", R);
        std::print("L is {}\n", L);
