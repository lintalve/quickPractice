...
public:
    ...
    Vector2& operator+(const Vector2& rv) {
        ...
        return *this; //bounding a temporary to a reference, why not const reference
    }
    Vector2& operator-(const Vector2& rv) {
        vector[0] += -(rv.vector[0]);
        vector[1] += -(rv.vector[1]);
...
public:
    ...
    Vector3& operator+(const Vector3& rv) {
        ...
        return *this;   //*this is an lvalue reference to the current object, not a temporary.
    }
    Vector3& operator-(const Vector3& rv) {
        vector[0] += -(rv.vector[0]);
        vector[1] += -(rv.vector[1]);
    ...
    Vector3& operator*(const double& rvdouble) {
        vector[0] *= static_cast<double>(rvdouble);
        vector[1] *= static_cast<double>(rvdouble);
        vector[2] *= static_cast<double>(rvdouble);
        return *this;
    }
    Vector3& operator*=(const int& rvint) {
        vector[0] *= rvint;
...
int main() {
    ...
    Vector3 vec7 = vec6;    //copy operator= 
