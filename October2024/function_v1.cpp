...
lint::Vector3 function1(lint::Vector3 vec1) {
    return vec1;
}
...
//1) you can assign const to a non const??
...
lint::Vector3 func6(lint::Vector3& vec) {   //here we breaking the reference by returning from the fanction 'by value', meaning a copy
                                            //remember if not a reference means copying. So RVO rules apply or mandatory copy elision apply
    return vec;                             //we are not creating a temporary object, we are not creating copy of incoming object,
